...
def divideTrainValidationTest(source_path, train_path, validation_path, test_path):
    ...
    """
    先将一类图像的路径拿出来，将这个路径下所有这类的图片，使用os.listdir函数，
    然后再将列表里面的所有图像名进行shuffle就是随机打乱，然后从打乱后的图像中抽6成放入训练集，2成放入验证集，2成
    放入测试集的图像名称列表
    """
    for i in range(0, classes_num):
        source_image_dir = os.listdir(source_path + classes_name_list[i] + '/')
        random.shuffle(source_image_dir)
        train_image_list = source_image_dir[0:int(0.6 * len(source_image_dir))]
        validation_image_list = source_image_dir[int(0.6 * len(source_image_dir)):int(0.8 * len(source_image_dir))]
        test_image_list = source_image_dir[int(0.8 * len(source_image_dir)):]
        """
...
if __name__ == '__main__':
    # 不用提前建文件夹，不然会出错
    data_path = 'D:/kagglecatsanddogs_5340/dataset/'  # train的上一级
    source_path = 'D:/kagglecatsanddogs_5340/dataset/'
    train_path = 'D:/kagglecatsanddogs_5340/dataset/train/'
    validation_path = 'D:/kagglecatsanddogs_5340/dataset/val/'
    test_path = 'D:/kagglecatsanddogs_5340/dataset/test/'
